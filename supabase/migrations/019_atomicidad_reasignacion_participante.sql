...
-- HuellAPP
-- Migración 019
-- Reasignación atómica de participantes
-- ============================================================
--
-- OBJETIVO
-- ------------------------------------------------------------
-- Evitar estados parciales al reasignar una participación.
--
-- La función centraliza en una sola transacción PostgreSQL:
-- - validación del actor y permiso REASSIGN_ASSIGNMENT;
-- - validación de asignación, participación y nuevo usuario;
-- - archivado de la participación anterior;
-- - creación de una nueva participación PENDIENTE;
-- - creación automática de asistencia PENDIENTE mediante el trigger existente;
-- - eventual transición CONFIRMADA -> PENDIENTE si desaparece el último
--   RELATOR ACEPTADO activo;
-- - audit_logs de la reasignación y, cuando corresponda, del cambio de estado.
--
-- La participación anterior y su asistencia se conservan como historial.
...
-- ============================================================
-- 1. FUNCIÓN TRANSACCIONAL DE REASIGNACIÓN
-- ============================================================

CREATE OR REPLACE FUNCTION public.reasignar_participante_atomico(
...
BEGIN
    ...
    -- --------------------------------------------------------
    -- FastAPI ya exige REASSIGN_ASSIGNMENT. Esta validación se
    -- repite en PostgreSQL como defensa en profundidad porque la
    -- función usa SECURITY DEFINER.
    -- --------------------------------------------------------
    ...
    -- --------------------------------------------------------
    -- BLOQUEAR ASIGNACIÓN DURANTE TODA LA OPERACIÓN
    -- --------------------------------------------------------
    ...
    IF v_participacion.usuario_id = p_nuevo_usuario_id THEN
        RETURN jsonb_build_object(
            'ok', false,
    ...
    IF NOT FOUND THEN
        RETURN jsonb_build_object(
            'ok', false,
            'error_code', 'NEW_USER_NOT_FOUND'
        );
    END IF;

    IF v_nuevo_usuario.rol_codigo NOT IN (
        'DIRECTIVA',
        'COORDINADOR',
        'MONITOR'
    ) THEN
        RETURN jsonb_build_object(
            'ok', false,
            'error_code', 'NEW_USER_CANNOT_PARTICIPATE'
        );
    END IF;
    ...
    IF EXISTS (
        ...
        WHERE ap.asignacion_id = p_asignacion_id
          AND ap.usuario_id = p_nuevo_usuario_id
          AND ap.activo = true
          AND ap.deleted_at IS NULL
    ) THEN
        RETURN jsonb_build_object(
            'ok', false,
            'error_code', 'NEW_USER_ALREADY_PARTICIPATES'
        );
    END IF;


    -- --------------------------------------------------------
    -- VALIDAR TIPO Y ESTADO PENDIENTE
    -- --------------------------------------------------------
    ...
    -- --------------------------------------------------------
    -- PREPARAR AUDITORÍA DEL REGISTRO ANTERIOR
    -- --------------------------------------------------------
    ...
    -- --------------------------------------------------------
    -- ARCHIVAR PARTICIPACIÓN ANTERIOR
    -- --------------------------------------------------------
    ...
    -- --------------------------------------------------------
    -- El trigger trg_create_pending_attendance crea su asistencia
    ...
    VALUES (
        p_actor_user_id,
        v_actor.rol_id,
        'REASSIGN_PARTICIPANT',
        'ASSIGNMENT_PARTICIPANT',
        v_nueva_participacion_id,
        v_old_values,
        v_new_values,
        'Participación reasignada a otro usuario. La participación anterior y su asistencia permanecen como historial.',
        p_request_id,
        p_ip_address,
        p_user_agent,
        'WEB'
    ...
    -- --------------------------------------------------------
    -- CONFIRMADA -> PENDIENTE SI YA NO HAY RELATOR ACEPTADO
    -- --------------------------------------------------------
...
COMMENT ON FUNCTION public.reasignar_participante_atomico(
    uuid,
    uuid,
    uuid,
    uuid,
...
'Reasigna una participación de forma atómica: archiva el registro anterior, crea uno nuevo PENDIENTE, conserva la asistencia histórica y audita todos los cambios asociados.';
...
REVOKE ALL ON FUNCTION public.reasignar_participante_atomico(
    uuid,
    uuid,
    uuid,
    uuid,
...
GRANT EXECUTE ON FUNCTION public.reasignar_participante_atomico(
    uuid,
    uuid,
    uuid,
    uuid,
